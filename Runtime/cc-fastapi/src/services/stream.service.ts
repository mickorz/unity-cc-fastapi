...
import type { ClaudeStreamData, SSEEvent } from '../types/index.js';
 ...
 * @returns SSE 格式的字符串
 */
export function toSSEFormat(event: SSEEvent): string {
 ...
 * @returns 解析结果
 */
function extractTextContent(data: Record<string, unknown>): ParseResult | null {
    // 处理 stream_event 类型
    if (data.type === 'stream_event' && data.event) {
        ...
        return { type: 'data', value: data as ClaudeStreamData };
    }
    ...
    if (data.type === 'result') {
        // 创建 result 对象的副本
        const resultCopy = { ...data } as Record<string, unknown>;
        ...
        return { type: 'data', value: resultCopy as ClaudeStreamData };
    }
 ...
 * @returns 解析后的数据对象或文本
 */
function parseLine(line: string): ClaudeStreamData | string | null {
    ...
    try {
        ...
        if (result.type === 'text') {
            ...
            return result.value as ClaudeStreamData;
        }
    } catch {
        // 非 JSON 格式，作为纯文本返回
        return trimmedLine;
 ...
 * @yields SSE 格式的字符串
 */
export async function* streamClaudeOutput(
 ...
 * @returns SSE 写入器
 */
export function createSSEWriter(write: (data: string) => void): SSEWriter {
