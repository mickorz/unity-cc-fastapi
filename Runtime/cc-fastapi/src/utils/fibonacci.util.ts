 ...
 * 缓存记忆化存储
 */
const memoCache = new Map<number, bigint>();
 ...
 * @returns 第 n 个斐波那契数
 */
export function fibonacciIterative(n: number): bigint {
    if (n < 0) {
 ...
 * @returns 第 n 个斐波那契数
 */
export function fibonacciMemoized(n: number): bigint {
    if (n < 0) {
        throw new Error('输入必须是非负整数');
    }
    if (n === 0) return 0n;
...
export function fibonacciMatrix(n: number): bigint {
    ...
    const matrixPower = (mat: [bigint, bigint, bigint, bigint], power: number): [bigint, bigint, bigint, bigint] => {
...
export function fibonacciSequence(
    n: number,
    method: 'iterative' | 'recursive' | 'memoized' | 'matrix' = 'iterative'
): bigint[] {
    if (n < 0) {
        throw new Error('输入必须是非负整数');
    }
...
export function fibonacci(
    n: number,
    method?: 'iterative' | 'recursive' | 'memoized' | 'matrix'
): bigint {
