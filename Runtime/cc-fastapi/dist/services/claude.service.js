 ...
 * @returns 参数
 */
export function getClaudeArgs(args) {
    return args;
}
/**
 * 获取 MCP 配置文件路径
 ...
 * @returns 命令行参数数组
 */
export function buildClaudeArgs(session, isStream, mcpConfigPath, permissionArgs) {
    // 基础参数
    const args = [
        '-p', // 从 stdin 读取 prompt
 ...
 * @returns 启动配置对象
 */
export function buildSpawnConfig(cwd, env) {
    return {
        cwd,
        env,
 ...
 */
export function spawnClaudeProcess(cwd, env, prompt, session = null, permissionArgs) {
    // 获取配置
