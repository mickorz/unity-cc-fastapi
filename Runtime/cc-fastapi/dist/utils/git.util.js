 ...
 */
export function cloneRepo(repo, targetDir) {
    ...
    if (clone.status !== 0) {
        ...
        throw new GitError(`克隆仓库失败: ${repo}`, stderr);
    }
 ...
 */
export function getTargetDirectory(session, repo, workingDir) {
    // 如果是恢复会话，使用现有工作目录
    if (session) {
        return workingDir;
    }
 ...
 */
export function getRepoUrl(dir) {
    if (!isGitRepo(dir)) {
        return null;
    }
